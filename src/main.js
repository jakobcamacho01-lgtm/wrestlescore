...
function saveUploadedMatch() {
  ...
  if (!ath) { toast('Please select an athlete first'); return; }
  ...
  // Reset filters so the new match is always visible
  libFilters = { event: 'All', athlete: 'All' };
...
function resLabel(res) {
  return res === 'W' ? 'W' : res === 'L' ? 'L' : res === 'F' ? 'FALL' : res;
}

function deleteMatch(idx, e) {
  e.stopPropagation();
...
function renderLib(q = '') {
  ...
  Object.keys(groups).sort().forEach(ev => {
    ...
    groups[ev].forEach(m => {
      ...
      const sc = (m.res === 'W' || m.res === 'F') ? 'var(--green)' : m.res === 'L' ? 'var(--red)' : 'var(--gold)';
      ...
      row.innerHTML = `<div class="li-thumb">${m.videoURL ? '<svg viewBox="0 0 24 24" width="14" height="14" fill="var(--green)"><polygon points="5 3 19 12 5 21 5 3"/></svg>' : '<svg viewBox="0 0 24 24" width="14" height="14" fill="rgba(255,255,255,.3)"><path d="M23 7l-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2" fill="rgba(255,255,255,.3)"/></svg>'}</div><div style="flex:1;min-width:0"><div class="li-names">${dot}${m.w1} vs. ${m.w2}</div><div class="li-meta">${m.wt} · ${m.ev}${m.videoURL ? ' · 🎬' : ''}</div></div><div style="text-align:right;flex-shrink:0"><div class="li-score" style="color:${sc}">${resLabel(m.res)} ${m.s1}–${m.s2}</div><div class="li-date">${m.dt}</div></div><button onclick="deleteMatch(${idx},event)" style="margin-left:8px;flex-shrink:0;width:30px;height:30px;border-radius:6px;border:1px solid var(--border-light);background:var(--black-3);display:flex;align-items:center;justify-content:center;cursor:pointer;-webkit-tap-highlight-color:transparent"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="var(--red)" stroke-width="2" stroke-linecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg></button>`;
...
function openMatch(idx) {
  ...
  const isWin = m.res === 'W' || m.res === 'F';
  res.textContent = m.res === 'W' ? 'WIN' : m.res === 'L' ? 'LOSS' : m.res === 'F' ? 'FALL' : 'DRAW';
  res.style.background = isWin ? 'rgba(29,185,84,.15)' : m.res === 'L' ? 'rgba(231,76,60,.15)' : 'rgba(240,180,41,.15)';
  res.style.color = isWin ? 'var(--green)' : m.res === 'L' ? 'var(--red)' : 'var(--gold)';
  res.style.borderColor = isWin ? 'rgba(29,185,84,.3)' : m.res === 'L' ? 'rgba(231,76,60,.3)' : 'rgba(240,180,41,.3)';
...
function exportMatchStats() {
  ...
  const lines = [
    ...
    'Score: ' + m.s1 + '–' + m.s2 + ' ' + (m.res === 'W' ? 'WIN' : m.res === 'L' ? 'LOSS' : m.res === 'F' ? 'FALL' : 'DRAW'),
    '',
